...
const getFallbackChallenge = (sex, usedIds) => {
  const pool = [
    ...CHALLENGES_GENERAL,
    ...(sex === "female" ? CHALLENGES_FEMALE : []),
...
export default function Ritual({ profile, targets, entries, waterMl, cyclePhase, activeChallenges, startChallenge, checkInChallenge, abandonChallenge }) {
  ...
  const generate = async (rejected = []) => {
    ...
    if (available.length > 0) {
      ...
      const genderCtx = sex === "female" ? "Female user. Women's wellness content is welcome."
      ...
      if (!challenge) challenge = getFallbackChallenge(sex, usedIds);
    }
