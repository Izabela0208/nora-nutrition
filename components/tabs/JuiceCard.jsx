...
export default function JuiceCard({ juice, isSaved, onToggleSave, ingChecked, onIngToggle }) {
    ...
    <div style={{ backgroundColor:G.card, borderRadius:16, border:`1px solid ${G.border}`, overflow:"hidden", boxShadow:"0 1px 5px rgba(27,58,45,0.05)" }}>
      <div style={{ display:"flex", alignItems:"stretch" }}>
        <button
          ...
          style={{ flex:1, padding:"14px 0 14px 16px", display:"flex", alignItems:"flex-start", gap:12, background:"none", border:"none", cursor:"pointer", textAlign:"left", minWidth:0 }}
        >
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
              <MacroChip label="kcal" value={juice.kcal} unit=""/>
              <MacroChip label="P" value={(juice.macros||{}).protein_g||0} unit="g"/>
              <MacroChip label="C" value={(juice.macros||{}).carbs_g||0} unit="g"/>
              <MacroChip label="F" value={(juice.macros||{}).fat_g||0} unit="g"/>
            </div>
          </div>
        </button>
          ...
          <div style={{ marginBottom:16 }}>
            {(juice.ingredients || []).map((ing, i) => {
              ...
              return (
                <div
                  key={i}
                  onClick={() => onIngToggle(`${juice.id}_${i}`)}
                  style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", cursor:"pointer", borderBottom: i < juice.ingredients.length-1 ? `1px solid ${G.border}` : "none", opacity: ck ? 0.4 : 1 }}
                >
